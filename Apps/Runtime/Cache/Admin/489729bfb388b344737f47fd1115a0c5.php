<?php if (!defined('THINK_PATH')) exit();?><body>
	<script type="text/javascript">
        var authRuleUrl="<?php echo U('Auth/ruleList');?>";
		var authRuleDelUrl="<?php echo U('Auth/ruleDel');?>";
        //var errorUrl='<?php echo U("Auth/rule");?>';
        var errorUrl='<?php echo U("Index/index");?>';
        //var errorUrl='<?php echo U("Index/error");?>';
		using('../../../default/js/authRule.js');
        /*$(function(){
            $.ajax({
                //async:false,
                type:"POST",
                url:'<?php echo U("Auth/rule");?>',
                data:'',
	...
	<div id="authRuleAddDialog" class="easyui-dialog" title="规则添加" style="width:500px;padding:10px 20px"
        closed="true" buttons="#authRuleAddButtons"  modal="true">
         <form id="authRuleAddForm" method="post" novalidate>
         	<div class="fitem">
                <label>规则标识</label>
                <input name="name" class="easyui-textbox" data-options="height:30,delay:1000,required:true,width:200,validType:['length[2,80]','exists[\'规则标识\',\'#authRuleId\',\'<?php echo U('Auth/checkRule','','');?>\']']"/>
            </div>
         	<div class="fitem">
                <label>规则简述</label>
            ...
            <div class="fitem">
               ...
               <select class="easyui-combobox" name="pid" id='authRuleMid' data-options="required:true,height:30,width:200,editable:false">	
               		<?php if(is_array($authRuleInfo)): $i = 0; $__LIST__ = $authRuleInfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['pid']); ?>"><?php echo ($vo['title']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
         </form>
    </div>
    <div id="authRuleAddButtons">
    	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="authRuleObj.add('<?php echo U('Auth/ruleAdd');?>')" style="width:90px">添加</a>
        ...
        closed="true" buttons="#authRuleEditButtons"  modal="true">
         <form id="authRuleEditForm" method="post" novalidate>
            <div class="fitem">
                ...
                <input name="name" class="easyui-textbox" data-options="height:30,required:true,width:200,validType:['length[2,80]','exists[\'规则标识\',\'#authRuleId\',\'<?php echo U('Auth/checkRule','','');?>\']']"/>
            </div>
            <div class="fitem">
                <label>规则简述</label>
                <input name="title" class="easyui-textbox" data-options="height:30,required:true,width:200,validType:['length[2,20]']" />
    ...
    <div id="authRuleEditButtons">
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="authRuleObj.save('<?php echo U('Auth/ruleSave');?>')" style="width:90px">添加</a>
